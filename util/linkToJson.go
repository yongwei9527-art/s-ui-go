...
func vless(u *url.URL, i int) (*map[string]interface{}, string, error) {
	...
	host, port := splitHostPortDefault(u.Host, defaultPort)
...
func hy2(u *url.URL, i int) (*map[string]interface{}, string, error) {
	...
	hopInterval := firstQueryValue(&query, "hop_interval", "hopInterval")
	if down > 0 {
		hy2["down_mbps"] = down
	}
	if up > 0 {
...
func tuic(u *url.URL, i int) (*map[string]interface{}, string, error) {
	...
	if congestionControl := firstQueryValue(&query, "congestion_control", "congestionControl"); congestionControl != "" {
		tuic["congestion_control"] = congestionControl
	}
	if udpRelayMode := firstQueryValue(&query, "udp_relay_mode", "udpRelayMode"); udpRelayMode != "" {
		tuic["udp_relay_mode"] = udpRelayMode
	}
	if queryBool(&query, "allow_insecure", "allowInsecure") {
		if tls, ok := tuic["tls"].(map[string]interface{}); ok {
			tls["insecure"] = true
		}
	}
	if queryBool(&query, "disable_sni", "disableSNI") {
		if tls, ok := tuic["tls"].(map[string]interface{}); ok {
...
func getTransport(tp_type string, q *url.Values) map[string]interface{} {
	...
	case "ws":
		...
		if earlyDataHeader := firstQueryValue(q, "ed", "early_data_header_name", "earlyDataHeaderName"); earlyDataHeader != "" {
			transport["early_data_header_name"] = earlyDataHeader
		}
		if maxEarlyData := firstQueryValue(q, "maxEarlyData", "max_early_data"); maxEarlyData != "" {
			if value, err := strconv.Atoi(maxEarlyData); err == nil && value > 0 {
...
func getTls(security string, q *url.Values) map[string]interface{} {
	...
	if len(tls_ech) > 0 {
		ech := map[string]interface{}{
			"enabled": true,
			"config":  queryCSV(q, "ech"),
		}
		if pq := firstQueryValue(q, "ech_pq_signature_schemes_enabled", "echPQSignatureSchemesEnabled"); pq != "" {
			ech["pq_signature_schemes_enabled"] = queryBool(q, "ech_pq_signature_schemes_enabled", "echPQSignatureSchemesEnabled")
		}
		if dynamicRecordSizingDisabled := firstQueryValue(q, "ech_dynamic_record_sizing_disabled", "echDynamicRecordSizingDisabled"); dynamicRecordSizingDisabled != "" {
			ech["dynamic_record_sizing_disabled"] = queryBool(q, "ech_dynamic_record_sizing_disabled", "echDynamicRecordSizingDisabled")
		}
		tls["ech"] = ech
	}
