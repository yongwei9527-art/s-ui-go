...
func addTls(out *map[string]interface{}, tls *model.Tls) {
	...
	if reality, ok := tlsServer["reality"].(map[string]interface{}); ok && enabledValue(reality, "enabled", false) {
		realityConfig := ensureMapValue(tlsConfig, "reality")
		realityConfig["enabled"] = true
		if shortIDs := stringSliceValue(reality["short_id"]); len(shortIDs) > 0 {
			realityConfig["short_id"] = shortIDs[common.RandomInt(len(shortIDs))]
		}
		tlsConfig["reality"] = realityConfig
	}
	if ech, ok := tlsServer["ech"].(map[string]interface{}); ok && enabledValue(ech, "enabled", false) {
		echConfig := ensureMapValue(tlsConfig, "ech")
		echConfig["enabled"] = true
		if configs := stringSliceValue(ech["config"]); len(configs) > 0 {
			echConfig["config"] = configs
		}
		if pqSignatureSchemesEnabled, ok := boolValue(ech, "pq_signature_schemes_enabled"); ok {
			echConfig["pq_signature_schemes_enabled"] = pqSignatureSchemesEnabled
		}
		if dynamicRecordSizingDisabled, ok := boolValue(ech, "dynamic_record_sizing_disabled"); ok {
			echConfig["dynamic_record_sizing_disabled"] = dynamicRecordSizingDisabled
		}
