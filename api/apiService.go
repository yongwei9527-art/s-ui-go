...
func (a *ApiService) LoadPartialData(c *gin.Context, objs []string) error {
	...
	for _, obj := range objs {
		...
		case "clients":
			...
			data[obj] = clients
		case "config":
			config, err := a.SettingService.GetNormalizedConfig()
			if err != nil {
				return err
			}
...
func (a *ApiService) GetDNSLeakGuardCheck(c *gin.Context) {
	config, err := a.SettingService.GetNormalizedConfig()
	if err != nil {
		jsonObj(c, nil, err)
		return
	}
	mode, err := a.SettingService.GetDNSLeakGuardMode()
	if err != nil {
		jsonObj(c, nil, err)
		return
	}
	report, err := service.BuildDNSLeakGuardCheckReport(json.RawMessage(config), mode)
...
func (a *ApiService) Login(c *gin.Context) {
	...
	loginResult, err := a.UserService.Login(c.Request.FormValue("user"), c.Request.FormValue("pass"), remoteIP)
	if err != nil {
		jsonMsg(c, "", err)
		return
	}
	...
	err = SetLoginUser(c, loginResult.Username, sessionMaxAge)
	if err == nil {
		logger.Info("user ", loginResult.Username, " login success")
	} else {
		logger.Warning("login failed: ", err)
	}

	jsonObj(c, loginResult, err)
}
