...
func (j *JsonService) GetJson(subId string, format string) (*string, []string, error) {
	...
	checks.RunDNSLinkage(jsonConfig)
	checks.Apply(&jsonConfig)
	...
	for _, inData := range inbounds {
		...
		if protocol == "shadowsocks" {
			...
			if method == "2022-blake3-aes-128-gcm" {
				pass, _ = protocolConfigString(configs, "shadowsocks16", "password")
			} else {
				pass, _ = protocolConfigString(configs, "shadowsocks", "password")
			}
		...
		checks.CheckUDPProtocol(protocol, inData, outbound)
		checks.CheckTransportDNSLinkage(protocol, inData, outbound)
...
func (j *JsonService) addDefaultOutbounds(outbounds *[]map[string]interface{}, outTags *[]string) {
	...
	if !hasOutboundTag(*outbounds, "dns-out") {
		outbound = append(outbound, map[string]interface{}{
			"type": "dns",
			"tag":  "dns-out",
		})
	}
...
func (j *JsonService) addOthers(jsonConfig *map[string]interface{}) error {
	...
	dnsLeakGuardMode, err := j.SettingService.GetDNSLeakGuardMode()
	if err != nil {
		return err
	}
	if len(othersStr) == 0 {
		if dnsLeakGuardMode != service.DNSLeakGuardModeOff {
			(*jsonConfig)["dns"] = service.DefaultDNSLeakGuardConfig("proxy")
			if err := service.EnsureRouteDNSHijack(route, dnsLeakGuardMode); err != nil {
				return err
			}
			if dnsMap, ok := (*jsonConfig)["dns"].(map[string]interface{}); ok {
				if err := service.ValidateRouteDefaultDomainResolver(dnsMap, route); err != nil {
					return err
				}
			}
		}
	...
	if dnsLeakGuardMode == service.DNSLeakGuardModeOff {
		delete(*jsonConfig, "dns")
	} else if dns, ok := othersJson["dns"]; ok {
		if dnsMap, ok := dns.(map[string]interface{}); ok {
			if err := service.EnsureDNSLeakGuard(dnsMap, dnsLeakGuardMode, "proxy"); err != nil {
				return err
			}
			dns = dnsMap
		}
...
func hasOutboundTag(outbounds []map[string]interface{}, tag string) bool {
...
func protocolConfigString(configs map[string]interface{}, key string, field string) (string, bool) {
