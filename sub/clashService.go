...
func (s *ClashService) GetClash(subId string, hostname string) (*string, []string, error) {
	...
	outbounds, outTags, err := s.getOutbounds(client.Config, inDatas, checks, hostname)
	if err != nil {
		return nil, nil, err
	}
...
func (s *ClashService) ConvertToClashMeta(outbounds *[]map[string]interface{}, basicConfig string) (string, error) {
	...
	for _, obMap := range *outbounds {
		...
		skipReality:
			...
			if ech, ok := tls["ech"].(map[string]interface{}); ok {
				...
				if echEnabled {
					if echConfig, ok := ech["config"].([]interface{}); ok {
						echParts := make([]string, 0, len(echConfig))
						for _, item := range echConfig {
							if value, ok := item.(string); ok && value != "" {
								echParts = append(echParts, value)
							}
						}
						if len(echParts) > 0 {
	...
	if len(proxyGroups) < 2 {
		return "", errors.New("invalid Clash proxy group template")
	}
