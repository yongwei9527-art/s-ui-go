<template>
  ...
  <v-card>
    ...
    <v-row class="sub-ext-row" align="start">
      <v-col cols="12" sm="6" md="3" lg="2" v-if="optionTun" class="switch-col">
        <v-switch color="primary" v-model="tun" :label="$t('setting.tun')" hide-details />
      </v-col>
      <v-col cols="12" sm="6" md="3" lg="2" v-if="optionTun" class="switch-col">
        <v-switch color="primary" v-model="tunStrictRoute" label="Strict Route" hide-details />
      </v-col>
      <v-col cols="12" sm="6" md="3" lg="2" v-if="optionTun" class="switch-col">
        <v-switch color="primary" v-model="tunDnsHijack" label="DNS hijack" hide-details />
      </v-col>
      <v-col cols="12" sm="6" md="3" lg="2" v-if="optionDns" class="switch-col">
        <v-switch color="primary" v-model="dns" :label="$t('pages.dns')" hide-details />
      </v-col>
    </v-row>
    <v-row class="sub-ext-row" v-if="optionTun" align="start">
      <v-col cols="12">
    ...
    <v-row class="sub-ext-row" v-if="optionRules" align="start">
...
export default {
  ...
  computed: {
    ...
    tunStrictRoute: {
      get() { return this.metaJson['tun']?.['strict-route'] ?? false },
      set(v:boolean) { this.updateMetaJson({ ...this.metaJson['tun'], 'strict-route': v }, 'tun') }
    },
    tunDnsHijack: {
      get() { return (this.metaJson['tun']?.['dns-hijack']?.length ?? 0) > 0 },
      set(v:boolean) { this.updateMetaJson({ ...this.metaJson['tun'], 'dns-hijack': v ? ['any:53'] : [] }, 'tun') }
    },
    rules: {
