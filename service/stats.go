...
func (s *StatsService) SaveStats(enableTraffic bool) error {
	...
	if len(*stats) == 0 {
		s.setOnlines(currentOnlineResources)
		return nil
	}
	...
	for _, stat := range *stats {
		...
		if stat.Direction {
			...
			case "inbound":
				currentOnlineResources.Inbound = append(currentOnlineResources.Inbound, stat.Tag)
			case "outbound":
				currentOnlineResources.Outbound = append(currentOnlineResources.Outbound, stat.Tag)
			case "user":
				currentOnlineResources.User = append(currentOnlineResources.User, stat.Tag)
			}
		}
	}
	s.setOnlines(currentOnlineResources)
...
func (s *StatsService) setOnlines(resources onlines) {
	onlineResourcesMu.Lock()
	defer onlineResourcesMu.Unlock()
	onlineResources.Inbound = append([]string(nil), resources.Inbound...)
	onlineResources.Outbound = append([]string(nil), resources.Outbound...)
	onlineResources.User = append([]string(nil), resources.User...)
}
