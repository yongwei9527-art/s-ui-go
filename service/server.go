...
func (s *ServerService) GetCpuPercent() float64 {
	...
	if len(percents) == 0 {
		logger.Warning("can not find cpu percent")
		return 0
	}
	return percents[0]
}
...
func (s *ServerService) GetSystemInfo() map[string]interface{} {
	...
	if hostInfo, err := host.Info(); err == nil {
		versionParts := make([]string, 0, 2)
		if hostInfo.Platform != "" {
			versionParts = append(versionParts, hostInfo.Platform)
		}
		if hostInfo.PlatformVersion != "" {
			versionParts = append(versionParts, hostInfo.PlatformVersion)
		}
		if len(versionParts) > 0 {
			systemVersion = strings.Join(versionParts, " ")
		} else if hostInfo.KernelVersion != "" {
			systemVersion = runtime.GOOS + " " + hostInfo.KernelVersion
		}
	} else {
